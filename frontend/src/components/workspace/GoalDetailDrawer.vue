...
.detail-drawer { position: absolute; top: 16px; right: 16px; bottom: 16px; width: min(584px, calc(100% - 32px)); max-height: 920px; display: flex; flex-direction: column; overflow: hidden; color: var(--ink); background: var(--paper); border: 1px solid rgba(255,255,255,.8); border-radius: var(--radius-sm); box-shadow: 0 24px 100px rgba(28,29,53,.24); font-family: var(--text-cn); }
...
.header-mark { display: grid; place-items: center; width: 36px; height: 40px; color: var(--ink-500); background: linear-gradient(150deg, var(--canvas-soft), #f9eef2); border: 1px solid var(--line); border-radius: var(--radius-sm); font-size: 26px; font-weight: 400; }
...
.drawer-header button { width: 36px; height: 36px; padding: 10px; color: var(--ink-600); background: var(--paper); border: 1px solid var(--line); border-radius: 50%; }
.drawer-header button:hover { background: var(--canvas-soft); transform: rotate(90deg); }
.detail-scroll { min-height: 0; overflow-y: auto; overscroll-behavior: contain; scrollbar-width: thin; scrollbar-color: var(--line) transparent; }
.goal-title-block { position: relative; isolation: isolate; overflow: hidden; padding: 26px 28px 24px; background: radial-gradient(ellipse at 100% 15%, var(--canvas-soft), transparent 65%), linear-gradient(135deg, var(--paper), #fbf4f7); border-bottom: 1px solid var(--line); }
.goal-title-block.blue { background: radial-gradient(ellipse at 100% 15%, #ddeaf3, transparent 65%), linear-gradient(135deg, var(--paper), #f6f8fb); }
.goal-title-block.sage { background: linear-gradient(125deg, #eff6f2, var(--paper)); }
.title-topline { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: center; gap: 12px; }
.eyebrow { color: var(--ink-500); font-family: var(--display); font-size: 9px; font-weight: 500; letter-spacing: .17em; }
.title-ornament { position: relative; height: 65px; margin: 4px 0 0; pointer-events: none; }
.title-ornament span { position: absolute; width: 160px; height: 49px; top: 3px; right: -3px; border: 1px dashed var(--line); border-radius: 50%; transform: rotate(-18deg); }
...
.chapter-label { display: block; margin-top: -20px; color: var(--ink-500); font-size: 12px; letter-spacing: .04em; }
...
.goal-description { padding: 1px 0 8px 14px; margin: -4px 0 10px; border-left: 2px solid var(--line); }
.goal-description p { margin: 0 0 7px; color: var(--ink-600); font-size: 13px; line-height: 1.8; overflow-wrap: anywhere; }
.created-date { display: flex; align-items: center; gap: 7px; margin: 0; color: var(--ink-500); font-size: 12px; }
...
.section-icon { display: grid; place-items: center; width: 25px; height: 25px; color: var(--ink-500); background: var(--canvas-soft); border-radius: var(--radius-sm); font-size: 16px; }
...
.detail-notes dt i { width: 5px; height: 5px; border-radius: 50%; background: var(--accent-mid); }
...
.missing-note > span { font-size: 16px; color: var(--ink-500); }
.lifecycle-card { margin: 24px 28px 0; padding: 18px; background: linear-gradient(115deg, var(--canvas-soft), var(--paper)); border: 0; border-radius: var(--radius-sm); box-shadow: inset 0 1px 0 white; }
...
.lifecycle-card ol::before { content: ''; position: absolute; top: 14px; left: 16%; right: 16%; height: 1px; background: var(--accent-soft); }
.lifecycle-card li { position: relative; display: grid; justify-items: center; gap: 9px; color: var(--ink-500); font-size: 11px; }
.lifecycle-card li i { display: grid; place-items: center; width: 29px; height: 29px; border-radius: 50%; border: 1px solid var(--line); background: var(--paper); font-family: var(--display); font-size: 10px; font-style: normal; }
.lifecycle-card li.active { color: var(--ink-700); font-weight: 600; }
.lifecycle-card li.active i { color: white; background: linear-gradient(140deg, var(--accent-mid), var(--accent)); border-color: transparent; box-shadow: 0 0 0 4px var(--accent-soft); }
.lifecycle-card li.done i { color: var(--ink-500); background: var(--canvas-soft); border-color: var(--line); }
.lifecycle-card > p { margin: 0; padding-top: 13px; color: var(--ink-500); border-top: 1px solid var(--line); font-size: 12px; line-height: 1.8; }
.detail-timestamp { display: flex; padding: 20px 28px 0; }
.detail-assistant { width: calc(100% - 56px); display: flex; align-items: center; gap: 12px; margin: 20px 28px 0; padding: 14px 15px; text-align: left; border: 0; border-radius: var(--radius-sm); background: linear-gradient(110deg, var(--canvas-soft), #f8eef3); }.detail-assistant:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 6px 16px color-mix(in srgb, var(--shadow-color) 8%, transparent); }.detail-assistant:disabled { opacity: .5; }.assistant-spark { font-size: 28px; color: var(--ink-500); }.detail-assistant strong, .detail-assistant small { display: block; }.detail-assistant strong { color: var(--ink-700); font-size: 13px; font-weight: 500; }.detail-assistant small { margin-top: 5px; color: var(--ink-500); font-size: 11px; line-height: 1.7; }.detail-assistant > i { margin-left: auto; color: var(--ink-500); font-size: 20px; font-style: normal; }
.detail-endnote { display: flex; justify-content: center; align-items: center; gap: 8px; margin: 24px 28px; color: var(--ink-500); }
.detail-endnote > span { width: 28px; height: 1px; background: var(--accent-soft); }
...
.drawer-footer button { min-height: 44px; padding: 0 17px; border-radius: var(--radius-sm); font-size: 13px; font-weight: 500; }
.ghost-button { color: var(--ink-600); background: white; border: 1px solid var(--line-strong); }
.continue-button { display: flex; justify-content: space-between; align-items: center; gap: 20px; color: white; background: linear-gradient(120deg, var(--accent), var(--accent)); border: 1px solid var(--accent); box-shadow: 0 4px 10px color-mix(in srgb, var(--shadow-color) 14%, transparent); }
.continue-button > span { font-size: 18px; }
.continue-button:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 6px 16px color-mix(in srgb, var(--shadow-color) 21%, transparent); }
.continue-button:disabled { opacity: .5; }
.detail-loading { padding: 40px 28px; }
.detail-loading i { height: 16px; margin-bottom: 16px; display: block; background: var(--canvas-soft); border-radius: var(--radius-sm); animation: pulse 1s ease-in-out infinite alternate; }
...
@media (max-width: 620px) {
  .detail-drawer { top: 8px; right: 8px; bottom: 8px; width: calc(100% - 16px); border-radius: var(--radius-sm); }
