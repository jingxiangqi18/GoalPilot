...
<template>
  <section class="saved-plan-view" :class="{ embedded }" aria-label="已保存的正式计划">
    ...
    <PlanRoadmap v-else-if="plan" :plan="plan" read-only :goal-status="goalStatus" :pending-task="pendingTask" :task-busy="taskBusy" :task-feedback="taskFeedback" :task-updates-blocked="taskUpdatesBlocked" @update-task="updateTask" @refresh-tasks="refreshTasks" @open-library="$emit('back')" @reset="$emit('back')" @ask-assistant="$emit('ask-assistant', $event)" />
  </section>
</template>

<style scoped>
.saved-plan-view { display: grid; gap: 22px; }
.saved-plan-view.embedded { gap: 13px; }.embedded .source-goal { display: none; }.embedded .saved-plan-nav { padding-bottom: 5px; justify-content: space-between; }.embedded .saved-plan-nav button { padding-block: 6px; font-size: 11px; }.embedded .plan-loading, .embedded .plan-unavailable { padding: 32px 18px; }
.saved-plan-nav { display: flex; align-items: center; gap: 18px; padding: 0 0 16px; border-bottom: 1px solid color-mix(in srgb, var(--line) 50%, transparent); }
.saved-plan-nav button, .plan-unavailable button { padding: 10px 14px; border: 1px solid var(--line-strong); border-radius: var(--radius-sm); background: var(--paper); font-size: 12px; white-space: nowrap; }
.saved-plan-nav button { color: var(--ink-500); border: 0; border-radius: var(--radius-sm); background: #ffffff90; }
.saved-plan-nav button:hover:not(:disabled) { background: var(--canvas-soft); }
.saved-plan-nav .back-button { color: var(--ink-500); background: transparent; padding-left: 0; }
...
.plan-loading, .plan-unavailable { padding: 48px 24px; display: grid; justify-items: center; text-align: center; border: 1px solid var(--line-strong); border-radius: var(--radius-sm); background: linear-gradient(130deg, white, var(--paper), #faf4f6); }
.loading-symbol, .empty-symbol { width: 56px; height: 56px; display: grid; place-items: center; border-radius: var(--radius-sm); background: var(--coral-100); color: var(--coral-700); font-size: 27px; }
...
.loading-lines i { height: 50px; border-radius: var(--radius-sm); background: linear-gradient(90deg, var(--canvas-soft), var(--paper), var(--canvas-soft)); background-size: 200% 100%; animation: plan-shimmer 1.5s linear infinite; }
