...
try {
  ...
  assert.equal(await page.locator('.task-disclosure.open').count(), 0, 'Start with a scannable checklist, not a wall of task details')
  assert.equal(await page.locator('.stage-directory').isVisible(), false, 'Default view focuses on tasks')
  assert.equal(await page.locator('.plan-summary').isVisible(), false)
  assert.equal(await page.locator('.stage-objective').evaluate(el => el.open), false)
  await tasks.nth(0).click(); await tasks.nth(1).click()
  assert.equal(await page.locator('.task-disclosure.open').count(), 1, 'Only one task detail is open at a time')
  ...
  assert.match(await page.locator('.progress-legend').innerText(), /待开始\s*4/)
  assert.match(await page.locator('.progress-percent').innerText(), /25/)
  await showTasks()
  await page.locator('.task-filters').getByRole('button', { name: /待办/ }).click()
  assert.equal(await tasks.count(), 2)
  await showOverview(); await showTasks()
  assert.equal(await tasks.count(), 2, 'Switching reading views retains the task filter')
  ...
  await page.locator('.task-filters').getByRole('button', { name: /已跳过/ }).click()
  assert.equal(await tasks.count(), 1)
  assert.equal(await page.getByRole('checkbox').getAttribute('aria-checked'), 'false')
  await page.locator('.task-filters').getByRole('button', { name: /全部/ }).click()
  ...
  // Returning from the current directory item must not leave focus on a hidden button.
  await showOverview(); await directory.nth(0).click(); await settle()
  ...
  assert.equal(await tasks.nth(0).getAttribute('aria-expanded'), 'true')
  await tasks.nth(0).click()
  ...
  assert.equal(await page.locator('.task-empty p').innerText(), '这个阶段暂未安排任务。')
  await showOverview()
